...
--Muchas veces el usar una subconsulta, puede generar el mismo efecto que un LEFT o RIGHT JOIN, tal y como se ve a continuación:
SELECT p.prod_codigo, p.prod_detalle, (SELECT ISNULL(SUM(stoc_cantidad), 0) FROM STOCK WHERE stoc_producto = prod_codigo) AS Stock --Se incorporan los productos que no matchean con stock, por lo que su stoc_cantidad es desconocida, o sea NULL
FROM Producto p
ORDER BY p.prod_detalle ASC

--Obsérvese lo que sucede si se pone SUM(ISNULL(stoc_cantidad, 0)):
SELECT p.prod_codigo, p.prod_detalle, (SELECT SUM(ISNULL(stoc_cantidad, 0)) FROM STOCK WHERE stoc_producto = prod_codigo) AS Stock --Se incorporan los productos que no matchean con stock, por lo que su stoc_cantidad es desconocida, o sea NULL
FROM Producto p
ORDER BY p.prod_detalle ASC
...
--Esta fue una primera resolución (y en definitiva, la correcta):
...
/*La resolución dada por el ayudante (mal hecha, porque faltan 4 clientes, y son los que compraron, pero en otros años disitntos a 2012, y no en 2012)
es la siguiente:*/
SELECT c.clie_codigo AS 'Código de cliente',
       COUNT(DISTINCT (f.fact_numero + f.fact_sucursal + f.fact_tipo)) AS 'Cantidad de veces que compró en el último año', --OBS. 14 a
...
LEFT JOIN Factura f ON c.clie_codigo = f.fact_cliente
LEFT JOIN Item_Factura i ON i.item_numero = f.fact_numero AND i.item_sucursal = f.fact_sucursal AND i.item_tipo = f.fact_tipo
WHERE YEAR(f.fact_fecha) = (SELECT MAX(YEAR(fact_fecha)) FROM Factura) OR YEAR(fact_fecha) IS NULL
GROUP BY c.clie_codigo
ORDER BY 2 DESC
...
--Facturas incorrectas por año
SELECT YEAR(fact_fecha), fact_numero+fact_sucursal+fact_tipo, SUM(item_cantidad * item_precio) as 'suma de items'
FROM Item_Factura
JOIN Factura ON item_numero = fact_numero AND item_sucursal = fact_sucursal AND item_tipo = fact_tipo
GROUP BY YEAR(fact_fecha), fact_numero+fact_sucursal+fact_tipo, fact_total, fact_total_impuestos
...
/*Obsérvese que esta solución no es correcta, debido a que la tabla Composicion multiplica la atomicidad por 2, ya que cada producto compuesto posee dos 
componentes en este modelo. Esto hace que el cálculo de las unidades vendidas se duplique:*/
SELECT prod_codigo AS 'Código del producto',
       prod_detalle AS 'Nombre del producto',
	   SUM(item_cantidad) AS 'Unidades facturadas'
FROM Producto
...
SELECT YEAR(fact_fecha) AS 'Año',
       p.prod_familia AS 'Código de familia más vendida',
...
JOIN Item_Factura ON prod_codigo = item_producto
JOIN Factura f ON item_numero = f.fact_numero AND item_sucursal = f.fact_sucursal AND item_tipo = f.fact_tipo
WHERE p.prod_familia = (SELECT TOP 1 fam.fami_id
                       FROM Familia fam
...
--La cantidad de productos vendidos por año, por familia son:
SELECT fami_id, fami_detalle,  SUM(item_cantidad) AS 'Total de productos vendidos' FROM Familia
...
/*Esta es otra forma de hacerlo. Sin embargo debe prestarse atención a que la tabla Composicion multiplica la atomicidad por dos, ya que cada producto 
compuesto posee dos componentes en este modelo. El hecho de que esta solución sea también correcta está en que se elige el producto compuesto más vendido, y 
este es uno. En otro ejercicio, esto podría llevar a errores.*/
...
--Las facturas mal realizadas son las siguientes:
SELECT fact_numero+fact_sucursal+fact_tipo
FROM Factura
JOIN Item_Factura ON fact_numero = item_numero AND fact_sucursal = item_sucursal AND fact_tipo = item_tipo
JOIN Producto ON item_producto = prod_codigo
WHERE YEAR(fact_fecha) = 2011 AND MONTH(fact_fecha) = 11 AND prod_rubro = '0001'--El de afuera
GROUP BY fact_numero+fact_sucursal+fact_tipo
HAVING COUNT(prod_rubro) = 2
