...
BEGIN
RETURN (SELECT CASE WHEN COUNT(*) = 0 THEN 0
                         ELSE COUNT(*) + SUM(dbo.cantidad_de_empleados_a_cargo(empl_codigo)) END
       FROM Empleado WHERE empl_jefe = @codigo_empleado)
END
GO

--Una forma menos compacta (planteada inicialmente):
CREATE FUNCTION dbo.cantidad_de_empleados_a_cargo_v2(@codigo_empleado numeric(6))
RETURNS int
AS
BEGIN
	...
	SET @cantidad_de_empleados_a_cargo = (SELECT COUNT(*) FROM Empleado WHERE empl_jefe = @codigo_empleado) + 
	                                     (SELECT SUM(dbo.cantidad_de_empleados_a_cargo_v2(empl_codigo))
										 FROM Empleado WHERE empl_jefe = @codigo_empleado)
	END
...
SELECT empl_codigo AS 'Código de empleado',
       empl_nombre AS 'Nombre',
	   empl_apellido AS 'Apellido',
	   dbo.cantidad_de_empleados_a_cargo_v2(empl_codigo) AS 'Cantidad de empleados a cargo' FROM Empleado
...
DROP FUNCTION dbo.cantidad_de_empleados_a_cargo_v2
GO
